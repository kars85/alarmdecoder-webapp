...
class ProfileForm(Form):
    ...
    location = StringField(u'Location', [Length(max=64)])
...
class ImportSettingsForm(Form):
    import_file = FileField(u'Settings Archive', [DataRequired()])

    submit = SubmitField(u'Import')

class HostSettingsForm(Form):
    hostname = StringField(u'Hostname', [DataRequired(), Length(max=63)])
    submit = SubmitField(u'Save')
...
class UPNPForm(Form):
    internal_port = IntegerField(u'Internal Port', [DataRequired()], default=443, description=u'Internal Port to Forward To')
    external_port = IntegerField(u'External Port', [DataRequired()], default=random.randint(1200,60000), description=u'External Port to map to Internal Port')

    submit = SubmitField(u'Save')

class VersionCheckerForm(Form):
    version_checker_timeout = IntegerField(u'Timeout in Seconds', [DataRequired(), NumberRange(600)], default=600, description=u'How often to check for version updates')
...
class ExportConfigureForm(Form):
    ...
    email_address = StringField(u'Email Address', [Optional(), Length(max=255)], description=u'Email Address to Send Export to')
    local_file = BooleanField(u'Save to Local File?', default=True)
    local_file_path = StringField(u'Path to Save file', [Optional(), Length(max=255)], default=os.path.join(INSTANCE_FOLDER_PATH, 'exports'), description='Path on AlarmDecoder to Save Export')
