...
from wtforms.validators import DataRequired, Length, EqualTo, Email, NumberRange, Optional, NoneOf, ValidationError
...
class NetworkDeviceForm(FlaskForm):
    device_address = StringField('Address', validators=[DataRequired(), Length(max=255)], description='Hostname or IP address', default='localhost')
    device_port = IntegerField('Port', validators=[DataRequired(), NumberRange(min=1024, max=65535)], default=10000)
    ssl = BooleanField('Connect to encrypted ser2sock? (Experimental)')
    buttons = FormField(SetupButtonForm)

class SSLForm(FlaskForm):
    ca_cert = FileField('CA Certificate', validators=[DataRequired()], description='CA certificate for AlarmDecoder.')
    cert = FileField('Client Certificate', validators=[DataRequired()], description='Client certificate for this webapp.')
    key = FileField('Client Key', validators=[DataRequired()], description='Private key for the client certificate.')
    buttons = FormField(SetupButtonForm)

class SSLHostForm(FlaskForm):
    config_path = StringField('SER2SOCK Config Path', validators=[DataRequired(), PathExists()], default='/etc/ser2sock')
    device_address = StringField('Address', validators=[DataRequired(), Length(max=255)], description='Hostname or IP address', default='localhost')
    device_port = IntegerField('Port', validators=[DataRequired(), NumberRange(min=1024, max=65535), NoneOf([80, 443, 5000], message="Port is reserved by other services.")], default=10000)
    ssl = BooleanField('Encrypt ser2sock?')
    buttons = FormField(SetupButtonForm)

class LocalDeviceForm(FlaskForm):
    device_path = StringField('Device Path', validators=[DataRequired(), Length(max=255), PathExists()], description='Path to the AlarmDecoder device.', default='/dev/serial0')
...
class DeviceForm(FlaskForm):
    ...
    keypad_address = IntegerField('Keypad Address', validators=[DataRequired(), NumberRange(min=1, max=99)], default=18)
    address_mask = StringField('AlarmDecoder Address Mask', validators=[DataRequired(), Length(max=8), Hex()], default='FFFFFFFF')
    internal_address_mask = StringField('Webapp Address Mask', validators=[DataRequired(), Length(max=8), Hex()], default='FFFFFFFF')
...
class CreateAccountForm(FlaskForm):
    name = StringField('Username', validators=[DataRequired(), Length(max=64)], default='admin')
    email = StringField('Email Address', validators=[DataRequired(), Length(max=64), Email()], default='admin@example.com')
    password = PasswordField('Password', validators=[DataRequired(), Length(min=8, max=64), StrongPassword()])
    password_again = PasswordField('Confirm Password', validators=[DataRequired(), EqualTo('password', message="Passwords must match.")])
    submit = SubmitField('Create Admin Account')

class EmailSetupForm(FlaskForm):
    mail_server = StringField('SMTP Server', validators=[DataRequired(), Length(max=255)], default='localhost')
    mail_port = IntegerField('SMTP Port', validators=[DataRequired(), NumberRange(min=1, max=65535)], default=25)
    ...
    default_sender = StringField('Default Sender Address', validators=[DataRequired(), Length(max=255)], default='alarmdecoder@example.com', description='From address for outgoing emails')
    buttons = FormField(SetupButtonForm)
